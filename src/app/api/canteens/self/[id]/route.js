...
export async function PUT(req,{params}){
    ...
    let {name,profile_img,cover_img} = getDataFromForm(formData,"name","profile_img","cover_img");
    if(!name || !profile_img || !cover_img){
        return Response.json({message : "All fields are required"})
    };
    profile_img = await handleImageEdit('canteen_img',profile_img,process.env.DEFAULT_CANTEEN_PROFILE_IMAGE_PATH,canteen,"profile");
    cover_img = await handleImageEdit('canteen_img',cover_img,process.env.DEFAULT_CANTEEN_COVER_IMAGE_PATH,canteen,"cover");
